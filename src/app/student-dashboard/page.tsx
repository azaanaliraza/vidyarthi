...
// We use a dynamic import for pdfjs-dist inside the component to prevent build errors
import {
...
function Chatbot({ documents }: { documents: any[] }) {
    ...
    const handleSendMessage = async () => {
        ...
        try {
            // --- NEW: Dynamically import and use the PDF library on the client-side ---
...
function StudentDashboard({ dbUser }: { dbUser: any }) {
  ...
  return (
    <div className="flex flex-col min-h-screen w-full bg-slate-50">
      <header className="bg-blue-700 shadow-lg sticky top-0 z-40 text-white">
          ...
          <div className="space-y-6">
            <TabsContent value="overview" className="mt-0 space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div className="bg-blue-600 rounded-lg shadow-lg p-6 text-white"><div className="flex items-center justify-between"><div><p className="text-sm font-medium">{t.totalCertificates}</p><p className="text-3xl font-bold">{documents.length}</p></div><Award className="h-8 w-8" /></div></div>
                    <div className="bg-orange-600 rounded-lg shadow-lg p-6 text-white"><div className="flex items-center justify-between"><div><p className="text-sm font-medium">{t.employabilityScoreShort}</p><p className="text-3xl font-bold">N/A</p></div><Trophy className="h-8 w-8" /></div></div>
                    <div className="bg-green-600 rounded-lg shadow-lg p-6 text-white"><div className="flex items-center justify-between"><div><p className="text-sm font-medium">{t.profileCompletion}</p><p className="text-3xl font-bold">{profileCompletion}%</p></div><User className="h-8 w-8" /></div></div>
                    <div className="bg-purple-600 rounded-lg shadow-lg p-6 text-white"><div className="flex items-center justify-between"><div><p className="text-sm font-medium">{t.activeProjects}</p><p className="text-3xl font-bold">0</p></div><Briefcase className="h-8 w-8" /></div></div>
                </div>
                <Card><CardHeader><CardTitle className="flex items-center gap-2"><ListChecks className="h-5 w-5" />{t.recentActivity}</CardTitle><CardDescription>{t.recentActivityDesc}</CardDescription></CardHeader><CardContent><div className="space-y-4">{recentActivities.length === 0 ? (<p className="text-center text-gray-500">No recent activity.</p>) : (recentActivities.map((activity) => (<div key={activity.id} className="flex items-center gap-4 p-3 rounded-lg bg-slate-50"><activity.icon className={`h-5 w-5 ${activity.type === 'document' ? 'text-red-500' : 'text-blue-600'}`} /><div className="flex-1"><p className="text-sm font-medium">{activity.title}</p><p className="text-xs text-muted-foreground">{activity.timestamp}</p></div></div>)))}</div></CardContent></Card>
            </TabsContent>
            <TabsContent value="profile" className="mt-0 space-y-6">
                <Card><CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"><div><CardTitle>{t.personalInfo}</CardTitle><CardDescription>{t.profileInfoDesc}</CardDescription></div><Button onClick={() => setIsEditing(!isEditing)} variant="outline" className="w-full sm:w-auto">{isEditing ? <X className="mr-2 h-4 w-4" /> : <Edit className="mr-2 h-4 w-4" />}{isEditing ? t.cancel : t.edit}</Button></CardHeader><CardContent className="space-y-4"><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><Label htmlFor="name">{t.fullName}</Label><Input id="name" value={profileData.name} disabled={!isEditing} onChange={handleProfileChange}/></div><div><Label htmlFor="email">{t.emailAddress}</Label><Input id="email" value={profileData.email} disabled/></div><div><Label htmlFor="phone">{t.phoneNumber}</Label><Input id="phone" value={profileData.phone} disabled={!isEditing} onChange={handleProfileChange}/></div><div><Label htmlFor="dob">{t.dateOfBirth}</Label><Input id="dob" value={profileData.dob} disabled={!isEditing} onChange={handleProfileChange} type="date"/></div><div><Label htmlFor="aadhar">Aadhar Number</Label><Input id="aadhar" value={profileData.aadhar} disabled={!isEditing} onChange={handleProfileChange}/></div><div><Label htmlFor="address">{t.address}</Label><Input id="address" value={profileData.address} disabled={!isEditing} onChange={handleProfileChange}/></div><div><Label htmlFor="course">Current Course</Label><Input id="course" value={profileData.course} disabled={!isEditing} onChange={handleProfileChange}/></div><div><Label htmlFor="institution">Institution</Label><Input id="institution" value={profileData.institution} disabled={!isEditing} onChange={handleProfileChange}/></div><div><Label htmlFor="highestQualification">Highest Qualification</Label>{isEditing ? (<Select value={profileData.highestQualification} onValueChange={handleQualificationChange}><SelectTrigger id="highestQualification"><SelectValue placeholder="Select qualification" /></SelectTrigger><SelectContent><SelectItem value="Below 10th">Below 10th Standard</SelectItem><SelectItem value="10th">10th Standard</SelectItem><SelectItem value="12th">12th Standard</SelectItem><SelectItem value="Bachelor's">Bachelor's Degree</SelectItem><SelectItem value="Master's">Master's Degree</SelectItem><SelectItem value="PhD">PhD</SelectItem><SelectItem value="Job">Doing Job</SelectItem></SelectContent></Select>) : (<Input id="highestQualification" value={profileData.highestQualification} disabled />)}</div></div><div><Label htmlFor="bio">Bio</Label><Textarea id="bio" value={profileData.bio} disabled={!isEditing} onChange={handleProfileChange}/></div><div><Label>Skills</Label><div className="flex flex-wrap gap-2 mt-2">{profileData.skills.map((skill) => (<Badge key={skill} variant="secondary">{skill}{isEditing && <Button variant="ghost" size="icon" className="h-4 w-4 ml-1" onClick={() => removeSkill(skill)}><X className="h-3 w-3"/></Button>}</Badge>))}</div>{isEditing && (<div className="flex gap-2 mt-2"><Input value={newSkill} onChange={(e) => setNewSkill(e.target.value)} placeholder="Add a new skill" onKeyDown={(e) => e.key === 'Enter' && addSkill()}/><Button onClick={addSkill}><Plus className="h-4 w-4"/></Button></div>)}</div>{isEditing && (<div className="flex justify-end"><Button onClick={handleSaveProfile}><Save className="mr-2 h-4 w-4"/>{t.save}</Button></div>)}</CardContent></Card>
            </TabsContent>
            <TabsContent value="certificates" className="mt-0 space-y-6">
                <Card><CardHeader><CardTitle>Compulsory Documents</CardTitle><CardDescription>Please upload these required documents based on your qualification.</CardDescription></CardHeader><CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">{compulsoryDocs.map(docType => { const uploadedDoc = uploadedCompulsoryDocs.find(d => d.type === docType); return (<Card key={docType} className={`p-4 flex items-center justify-between ${uploadedDoc ? 'bg-green-50' : 'bg-red-50'}`}><div><p className="font-medium">{docType}</p>{uploadedDoc ? <Badge variant="secondary" className="bg-green-200 text-green-800">Uploaded</Badge> : <Badge variant="destructive">Missing</Badge>}</div>{uploadedDoc ? (<Button asChild variant="outline" size="sm"><a href={uploadedDoc.url!} target="_blank" rel="noopener noreferrer">View</a></Button>) : (<Button size="sm" onClick={() => { setUploadState({ name: docType, issuer: "N/A", file: null, type: docType }); setIsUploadDialogOpen(true); }}>Upload</Button>)}</Card>)})}</CardContent></Card>
                <Card><CardHeader><div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"><div><CardTitle>{t.myCertificates}</CardTitle><CardDescription>{t.myCertificatesDesc}</CardDescription></div><Button className="bg-blue-600 hover:bg-blue-700 text-white w-full sm:w-auto" onClick={() => { setUploadState({ name: "", issuer: "", file: null, type: "General Certificate" }); setIsUploadDialogOpen(true); }}><Plus className="h-4 w-4 mr-2" />{t.addCertificate}</Button></div></CardHeader><CardContent><div className="grid grid-cols-1 gap-4">{generalDocs.map((doc) => (<div key={doc._id} className="grid grid-cols-1 sm:grid-cols-[1fr_auto] items-start gap-x-6 gap-y-4 p-4 border rounded-lg"><div className="flex items-start gap-4 min-w-0"><Award className="h-8 w-8 text-blue-600 flex-shrink-0 mt-1" /><div className="flex-grow"><h3 className="font-medium leading-tight">{doc.fileName} {doc.status === 'Verified' && <CheckCircle className="h-4 w-4 text-blue-600 inline-block ml-1" />}</h3><p className="text-sm text-muted-foreground">{doc.issuer}</p></div></div><div className="flex items-center gap-2 w-full sm:w-auto justify-end"><Badge variant={doc.status === 'Verified' ? "default" : "secondary"}>{doc.status}</Badge><Button asChild variant="outline" size="sm"><a href={doc.url!} target="_blank" rel="noopener noreferrer">{t.view}</a></Button><Button variant="destructive" size="sm" onClick={() => handleDeleteDocument(doc._id)}><Trash2 className="h-4 w-4" /></Button></div></div>))}{generalDocs.length === 0 && <p className="text-center text-gray-500 py-4">No other certificates uploaded yet.</p>}</div></CardContent></Card>
            </TabsContent>
            <TabsContent value="employability" className="mt-0 space-y-6"><Card><CardHeader><CardTitle>{t.employabilityScore}</CardTitle><CardDescription>{t.employabilityScoreDesc}</CardDescription></CardHeader><CardContent className="text-center text-gray-500 py-8"><p>Employability score feature will be added soon.</p></CardContent></Card></TabsContent>
            <TabsContent value="internships" className="mt-0 space-y-6"><div className="text-center text-gray-500 py-8"><p>Internship listings coming soon.</p></div></TabsContent>
          </div>
        </Tabs>
      </main>
      <Dialog open={isUploadDialogOpen} onOpenChange={setIsUploadDialogOpen}>
        <DialogContent><DialogHeader><DialogTitle>Upload a new Document</DialogTitle><DialogDescription>{uploadState.type === "General Certificate" ? "Add details about your certificate below." : `Uploading: ${uploadState.type}`}</DialogDescription></DialogHeader><div className="space-y-4 py-4">{uploadState.type === "General Certificate" && (<><div><Label htmlFor="certName">Certificate Name</Label><Input id="certName" value={uploadState.name} onChange={(e) => setUploadState(s => ({ ...s, name: e.target.value }))} /></div><div><Label htmlFor="certIssuer">Issuing Authority</Label><Input id="certIssuer" value={uploadState.issuer ?? ""} onChange={(e) => setUploadState(s => ({ ...s, issuer: e.target.value }))} /></div></>)}<div><Label htmlFor="certFile">File</Label><Input id="certFile" type="file" onChange={(e) => setUploadState(s => ({...s, file: e.target.files?.[0] || null}))} /></div></div><DialogFooter><Button variant="outline" onClick={() => setIsUploadDialogOpen(false)}>Cancel</Button><Button onClick={() => handleFileUpload(uploadState.file!, uploadState.type, uploadState.issuer, uploadState.name)} disabled={!uploadState.file}><UploadCloud className="w-4 h-4 mr-2" /> Upload</Button></DialogFooter></DialogContent>
      </Dialog>
